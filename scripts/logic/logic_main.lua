...
skyward_sword_location = {}
skyward_sword_location.__index = skyward_sword_location
...
function skyward_sword_location.new(name)
    local self = setmetatable({}, skyward_sword_location)
    if name then
...
function skyward_sword_location:connect_one_way(exit, rule)
    if type(exit) == "string" then
        exit = skyward_sword_location.new(exit)
    end
    if rule == nil then
        rule = always
    end
...
function skyward_sword_location:connect_two_ways(exit, rule)
...
function skyward_sword_location:connect_one_way_entrance(name, exit, rule)
    if rule == nil then
        rule = always
    end
    self.exits[#self.exits + 1] = { exit, rule }
...
function skyward_sword_location:connect_two_ways_entrance(name, exit, rule)
...
-- acts as a shortcut for 2 connect_one_way_entrance-calls
function skyward_sword_location:connect_two_ways_entrance_door_stuck(name, exit, rule1, rule2)
...
function skyward_sword_location:accessibility()
...
-- 
function skyward_sword_location:discover(accessibility, keys)
...
entry_point = skyward_sword_location.new("entry_point")
